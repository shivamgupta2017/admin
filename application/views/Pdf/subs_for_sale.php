<?php 
$this->load->library('Fppdf');
$this->load->database();

$billing=[ucfirst($shipping_details[0]->vendor_name),$shipping_details[0]->address, $shipping_details[0]->city, $shipping_details[0]->zipcode,'GSTIN:'.$shipping_details[0]->gstin, $shipping_details[0]->email, $shipping_details[0]->mobile, $net_balance_to_pay];
...
$header = array('#', 'Item', 'Quantity', 'Unit','Unit price', 'Total Amount');
$data=[];

foreach($products as $index=>$temp_products)
{
	array_push($data,array($temp_products->product_name, $temp_products->quantity, 1, $temp_products->unit_name,$temp_products->cost, 0, '')); //[ ProductName, Qty, weight, unit, price,tax,isInclusive]
}
