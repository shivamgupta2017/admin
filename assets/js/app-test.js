		 ...
		 $scope.layoutsd = JSON.parse($scope.getresultsds[0].layout);});
		   ...
		   $scope.get_busname = function(){
			...
			$scope.get_set = function(){
				...
				$scope.loader=true;
				var url=base_url+'Seat_layout/get_max_seat';				 
				 $http({
					 url: url,  
					 method: "POST",				 
					 data: {'bus_id': $scope.bus},					 
				}).then(function mySucces(response) {
					$scope.noseat = response.data['max_seat'];
					$timeout(function() {
		    	        $scope.loader=false;
			        },1000);
				});
			...
			/*$scope.call_total = function(){				
				if($scope.noseat!==undefined && $scope.lastno!==undefined){
					$scope.total = parseInt($scope.noseat) + parseInt($scope.lastno);
					//alert($scope.total);
					if($scope.total_avaliable!=$scope.total){
						$('#error_msg').html('Maximum Seat no of this bus is '+$scope.total_avaliable);
						$scope.disabled_value = true;
					} else {
						//alert('equal');
						$scope.disabled_value = false;
					}
			...
			$scope.call_total = function(){				
				if($scope.noseat!==undefined && $scope.lastno!==undefined){
					$scope.total = parseInt($scope.noseat) - parseInt($scope.lastno);
					//alert($scope.total);
					/*if($scope.noseat!=$scope.total){
						$('#error_msg').html('Maximum Seat no of this bus is '+$scope.noseat);
						$scope.disabled_value = true;
					} else {
						//alert('equal');
						$scope.disabled_value = false;
			...
			/*$scope.$watch("noseat", function(newValue, oldValue){
				if()
			});*/
				
 $scope.get_bustypename = function(){

					//$scope.select_bustypes = [];
					//$scope.cinema_screen = response.data;
					//alert(JSON.stringify(cinema_screen.movie_name));
					$http.get(base_url+"Seat_layout/get_bustypename").success(function(data){ 
						$scope.select_bustypes = data; 
						//$scope.select_bustypes = response.data;
						//console.log(data);
					});
	            }
                ...
                $scope.loader=true;	
				$http.get(base_url+"Seat_layout/busNames").success(function(data){ 
						$scope.locationNames = data; 
						//$scope.locationNames = response.data;
						//console.log(data);
					});
